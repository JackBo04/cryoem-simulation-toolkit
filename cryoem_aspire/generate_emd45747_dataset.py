"""
Generate EMD-45747 Dataset
==========================
基于 EMD-45747 模型生成完整数据集：
- 模型尺寸: 320×320×320 @ 0.825 Å = 264 Å
- 图像尺寸: 400×400 (pad from 256)
- 像素大小: 1.031 Å
- 粒子半径: ~128 px (~132 Å)

包含:
1. clean_projections.mrcs - 无 CTF
2. ctf_projections.mrcs - 有 CTF + B-factor envelope
3. noisy_projections.mrcs - CTF + 噪声
4. r90, r110, r130, r150, r170 掩膜版本 (outside=noise)
"""
...
OUTPUT_DIR = Path("/mnt/data/zouhuangbo/cryo-EM/nrips/mask_test/cryoem_aspire/emd_45747_dataset")
OUTPUT_DIR.mkdir(exist_ok=True)

MAP_FILE = "/mnt/data/zouhuangbo/cryo-EM/nrips/mask_test/emd_45747.map"

# ========== 参数设置 ==========
N_PROJECTIONS = 10000
IMAGE_SIZE = 400
PROJECTION_SIZE = 256  # ASPIRE downsample 尺寸
PIXEL_SIZE = 1.031     # Å (264/256)
VOLTAGE = 300
CS = 2.7
AC = 0.1
SNR = 0.05
...
def generate_base_data():
    ...
    print("=" * 70)
    print("Generate EMD-45747 Base Data")
    print("=" * 70)
    print(f"Model: EMD-45747 (320×320×320 @ 0.825Å = 264Å)")
    print(f"Output size: {IMAGE_SIZE}×{IMAGE_SIZE}")
    print(f"Pixel size: {PIXEL_SIZE:.3f} Å")
    print(f"Particle radius: ~128 px (~132 Å)")
    ...
    # Pad to target size
    print(f"\n[4/6] Padding to {IMAGE_SIZE}×{IMAGE_SIZE}...")
    t0 = time.time()
    ...
    # Save base projections
    print("\n[8/6] Saving base projections...")
    for name, data in [("clean", proj_clean), ("ctf", proj_ctf), ("noisy", proj_noisy)]:
...
def create_visualization(base_data):
    ...
    im1 = ax1.imshow(proj_clean, cmap='gray', vmin=-0.1, vmax=0.1)
    ax1.set_title('Clean (No CTF)', fontsize=11, fontweight='bold')
    ax1.axis('off')
    plt.colorbar(im1, ax=ax1, fraction=0.046)
    
    ax2 = plt.subplot(3, 4, 2)
    im2 = ax2.imshow(proj_ctf, cmap='gray', vmin=-0.05, vmax=0.05)
    ax2.set_title('CTF (B-factor=60Å²)', fontsize=11, fontweight='bold')
    ax2.axis('off')
    plt.colorbar(im2, ax=ax2, fraction=0.046)
    
    ax3 = plt.subplot(3, 4, 3)
    im3 = ax3.imshow(proj_noisy, cmap='gray', vmin=-0.1, vmax=0.1)
    ax3.set_title(f'Noisy (SNR={SNR})', fontsize=11, fontweight='bold')
    ax3.axis('off')
    ...
    im4 = ax4.imshow(ctf_effect, cmap='RdBu_r', vmin=-0.05, vmax=0.05)
    ax4.set_title('CTF Effect', fontsize=11, fontweight='bold')
    ax4.axis('off')
    plt.colorbar(im4, ax=ax4, fraction=0.046)
    
    # Row 2: Masks
    for idx, radius in enumerate(RADII[:4]):
    ...
    # Row 3: Last mask + cross-sections
    ax = plt.subplot(3, 4, 9)
    im = ax.imshow(masks[RADII[-1]], cmap='viridis', vmin=0, vmax=1)
    effective_r = RADII[-1] + APODIZATION_WIDTH
    particle_r = 128
    coverage = (effective_r * 2) / (particle_r * 2) * 100
    ax.set_title(f'R={RADII[-1]}px ({coverage:.0f}% coverage)', fontsize=10)
    ax.axis('off')
    plt.colorbar(im, ax=ax, fraction=0.046)
    ...
    ax.hist(proj_clean.ravel(), bins=50, alpha=0.5, label='Clean', color='blue', density=True)
    ...
    for name, proj, color in [('Clean', proj_clean, 'blue'), 
                               ('CTF', proj_ctf, 'green'), 
    ...
    plt.suptitle(f'EMD-45747 Dataset Overview\n'
                 f'Image: {IMAGE_SIZE}×{IMAGE_SIZE}px | Pixel: {PIXEL_SIZE:.3f}Å | '
                 f'Particle: ~264Å (~256px) | B-factor: {B_FACTOR}Å²', 
...
def create_readme():
    ...
    with open(readme_path, 'w') as f:
        f.write(f"""EMD-45747 Dataset
=================

Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}
Model: EMD-45747 (BRCA1)

Model Information:
------------------
- Original: 320×320×320 voxels @ 0.825 Å/pixel
- Physical size: 264.0 Å
- After downsample(256): 256×256×256 @ 1.031 Å/pixel
...
Particle Size:
--------------
- Particle diameter in image: ~256 px (~264 Å)
- Particle radius: ~128 px (~132 Å)
- Image padding: {(IMAGE_SIZE - 256) // 2}px on each side

Radius Scan (5 values):
-----------------------
{RADII}
...
- R=170: 370px effective = 145% coverage

Generated Files:
----------------
Base projections:
  - clean_projections.mrcs  (No CTF)
...
def main():
    ...
    print("=" * 70)
    print("EMD-45747 Dataset Generation")
    print("=" * 70)
